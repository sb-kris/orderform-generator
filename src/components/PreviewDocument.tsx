...
export function PreviewDocument() {
  ...
  return (
    <motion.div
      key={`prev-${data.customer.legalName}-${model.total}-${currency}`}
      initial={{ opacity: 0.85 }}
      animate={{ opacity: 1 }}
      transition={{ duration: 0.15 }}
      className="doc overflow-hidden rounded-lg border border-slate-200 bg-white shadow-card"
    >
            ...
            <div className="flex items-start justify-between">
              ...
              <div className="text-right">
                ...
                <div className="mt-1.5 text-[10px] text-slate-500">
                  Doc ID{' '}
                  <span className="font-mono text-slate-800">{data.documentId}</span>
                </div>
              </div>
            </div>
            <div className="mt-5 text-[10px] font-semibold uppercase tracking-[0.14em] text-teal-700">
              SurveySparrow Inc
            </div>
      ...
      <PreviewSection num="01" title="Customer Information">
        <PreviewFieldRow>
          <PreviewField label="Customer’s Legal Name" value={data.customer.legalName} required />
          <PreviewField label="Order Form Date" value={formatDate(data.customer.orderFormDate)} required />
        </PreviewFieldRow>
        <PreviewFieldRow>
          <PreviewField label="Pricing Valid Through" value={formatDate(data.customer.pricingValidThrough)} required />
          <PreviewField label="Prepared By" value={data.customer.preparedBy} required />
        </PreviewFieldRow>
      </PreviewSection>

      <PreviewSection num="02" title="Sold To">
        <PreviewFieldRow>
          <PreviewField label="Name" value={data.soldTo.name} required />
          <PreviewField label="Email" value={data.soldTo.email} required />
        </PreviewFieldRow>
      </PreviewSection>

      <PreviewSection num="03" title="Services">
        <table>
          <thead>
            <tr>
          ...
          <tbody>
            {populated.length === 0 ? (
              ...
              populated.map((l) => {
                ...
                return (
                  <tr key={l.id}>
                    <td className={cn(l.description && 'filled')}>
                      {l.description || '—'}
                    </td>
      ...
      <PreviewSection num="04" title="Billing & Shipping">
        <div className="grid grid-cols-2 gap-3">
          ...
          <AddressCard
            ...
            address={
              data.billing.sameAsBillTo ? data.billing.billTo.address : data.billing.shipTo.address
            }
          />
        </div>
      </PreviewSection>

      <PreviewSection num="05" title="Subscription Details">
        <PreviewFieldRow>
          <PreviewField label="Billing Period" value={data.subscription.billingPeriod} required />
          <PreviewField
        ...
        <PreviewFieldRow>
          <PreviewField label="Start Date" value={formatDate(data.subscription.startDate)} required />
          <PreviewField label="Payment Method" value={data.subscription.paymentMethod} required />
        </PreviewFieldRow>
      ...
      <PreviewSection num="06" title="Terms & Conditions">
        <div className="space-y-3">
          {TERMS.map((t) => (
            <div key={t.title}>
              <div className="text-[11.5px] font-semibold text-slate-950">{t.title}</div>
              {t.paragraphs.map((p, j) => (
                <p key={j} className="text-[10.5px] leading-snug text-slate-700">
                  {p}
                </p>
              ))}
            </div>
      ...
      <PreviewSection num="07" title="Execution / Signature">
      ...
      <PreviewSection num="08" title="Purchase Order">
        <div className="grid grid-cols-3 gap-3">
          <PreviewField label="PO Required" value={data.purchaseOrder.required} />
          <PreviewField
            label="PO Number"
...
function PreviewFieldRow({ children }: { children: React.ReactNode }) {
...
function AddressCard({
  ...
  return (
    <div className="rounded-lg border border-slate-200 p-3">
      ...
      <div className="mt-2 text-[10px] font-semibold uppercase tracking-[0.1em] text-slate-500">
        Name
      </div>
...
function SignatureBlock({
  ...
  title: string
  block: { signatureName: string; name: string; designation: string; date: string }
}) {
  return (
    <div className="rounded-lg border border-slate-200 p-3">
      <div className="text-[10px] font-semibold uppercase tracking-[0.14em] text-teal-700">
      ...
      <div className="mt-4 border-b border-slate-400 pb-1 font-[cursive] italic text-slate-800 min-h-[22px]">
        {block.signatureName || <span className="text-slate-300">Signature</span>}
      </div>
      <div className="mt-2 grid gap-1 text-[11px]">
        <div>
          <span className="doc-field-label">Name</span>
          <div className={cn('doc-field', block.name && 'filled')}>
            {block.name || '{{Name}}'}
          </div>
        </div>
        <div>
...
function BirdMark() {
  // Inline the SurveySparrow bird glyph (same paths as SurveySparrow_Symbol_0.svg)
  // Kept as an SVG so it stays crisp regardless of pixel ratio.
  return (
